...
linkedList_h* createLinkedList_h(void) {
	linkedList_h*L;			// linkedList_h가 리스트L을 가리킴
	L = (linkedList_h*)malloc(sizeof(linkedList_h));
	L->head = NULL;  // 공백 리스트 이므로 NULL로 설정   // 화살표 연산자 사용
	return L;
}
...
void insertFirstNode(linkedList_h*L, const char*x) {
	...
	strcpy_s(newNode->data, x);		// strcpy_s(주소 값, 저장할 데이터 값)
	newNode->link = L->head;
	L->head = newNode;
}
// 노드를  pre 뒤에 삽입하는 연산
...
void insertLastNode(linkedList_h*L, const char*x) {
	...
	newNode = (listNode*)malloc(sizeof(listNode));
	strcpy_s(newNode->data, x);
	newNode->link = NULL;    // 마지막 노드이기 때문에 NULL값 저장
...
listNode* searchNode(linkedList_h*L, const char*x) {
	...
	temp = L->head;
	while (temp != NULL) {
		if (strcmp(temp->data, x) == 0) return temp;						// strcmp(str1, str2) 가 리턴값이 0일 경우 같은 문자열 /  0이 아닐 경우 다른 문자열
		else temp = temp->link;
	}
	return temp;
}
