...
unsigned long int hash(const char* key, int size) {
	unsigned long hashvalue = 0;
	for (hashvalue; *key != NULL; key++)
		hashvalue += *key;
...
int Add(ChainHash* h, const Member* x) {	// 포인터 x가 가리키는 데이터를 추가
	int key = hash(x->name, h->size);	// 추가하는 데이터의 해시 값	( 해시 함수가 키값을 해시 값으로 변환)
	Node* p = h->table[key];	// 현재 선택한 노드	(해시 값을 인덱스로 하는 버킷을 선택)
	Node* temp;
	while (p != NULL) {	// 버킷에 저장된 포인터가 가리키는 연결리스트를 처음부터 순차적으로 검색
		if (!strcmp(p->data.name, x->name))	// 이미 등록된 키가 있을 경우
			return 1;	// 추가 실패
		p = p->next;		// 다음 노드를 선택
...
int Remove(ChainHash* h, const Member* x) {
	int key = hash(x->name, h->size);		// 삭제하는 데이터의 해시 값 (해시 함수가 키 값을 해시 값으로 변환)
	Node* p = h->table[key];				// 현재 선택한 노드 (해시 값을 인덱스로 하는 버킷을 선택)
	Node** pp = &h->table[key];			// 현재 선택한 노드에 대한 포인터
	while (p != NULL) {	// 선택한 버킷의 포인터가 가리키는 연결 리스트를 순차적으로 검색
		if (!strcmp(p->data.name, x->name)) {	// 키 값과 같은 값을 찾으면 
